...
| `streaming` _boolean_ | Streaming enables long-lived connection support for this NebariApp.<br />When true, the operator emits an Envoy Gateway BackendTrafficPolicy<br />targeting this app's operator-owned HTTPRoutes that disables the default<br />HTTP request timeout (15s -> no limit) and sets the connection idle<br />timeout to 5 minutes. Use this for Server-Sent Events (SSE), long-poll<br />endpoints, gRPC streaming, or any workload that holds a connection open<br />beyond a few seconds. When false or omitted, no policy is created and<br />Envoy's default timeouts apply. |  | Optional: \{\} <br /> |
